...
public class SessaoTest {
	...
	@Test
	public void oPrecoSessaoDeveSerPrecoSalaMaisPrecoFilme(){
		
		Sala sala = new Sala("Eldorado - IMax",new BigDecimal("22.5"));
		
		Filme filme = new Filme("RogueOne",Duration.ofMinutes(120),"SCI-FI",new BigDecimal("12.0"));
		
		BigDecimal somaPrecos = sala.getPreco().add(filme.getPreco());
		
		Sessao sessao = new Sessao(LocalTime.parse("10:00:00"), filme, sala);
		
		Assert.assertEquals(somaPrecos, sessao.getPreco());
	}
	
	@Test
	public void garanteLugarA1EstaOcupadoELugaresA2EA3Disponiveis() {
